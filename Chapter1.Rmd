...
1. Using the built-in dataset `ToothGrowth` in R, visualize thet data and comment on the effectiveness of different functions in the context.
...
`head()`: directly see how the dataset looks; useful when the dataset is large and it difficult to display all rows and columns together.

`summary()`: gives a brief summary of the data with numbers. Note that `fivenum()` works in a similar way but it only takes in a vector/column.

`boxplot()`: visualizes the five points. It's clear that the data is not skewed.
...
`hist()`: frequency plot, similar to `stem` but simpler.

`cumsum()`: no much useful information.
