...
1. Randomly choose one number from the distribution:
...
2. Take the average of 1000 picks:
...
3. Draw a histogram of the 1000 picks:
```{r}
...
1. Define a function for the cdf (you have to do the integration yourself):
...
We use `pnorm` to find the probability (area under the normal curve for Z < z) and `qnorm` to find the percentile (the value z for a given probability).
